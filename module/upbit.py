...
def get_macd(target_item, tick_kind, inq_range):
    try:
        ...
        for i in range(len(df) - 1, -1, -1):
            ...
            if i == 0:
                ...
                next_ocl = round(macd[i - 1] - exp3[i - 1], 4)
            
            if i == 0:
                ocl_change = ocl
            else:
                ocl_change = abs(next_ocl - ocl)
            
            # OCL 값이 양수에서 음수로 변하는 시점 (매도 시점) 
            if ocl >= 0 and next_ocl < 0 and quantity > 0:
                sell_price = df.iloc[i]
                revenue = quantity * sell_price
                profit = revenue - initial_investment
                final_capital = revenue  # 최종 자본금 업데이트
                trades.append({
                    "type": "sell",
                    "date": candle_data[i]['candle_date_time_kst'],
                    "price": sell_price,
                    "revenue": revenue,
                    "profit": profit
                })
                print(f"매도 at {candle_data[i]['candle_date_time_kst']} with price {sell_price}, revenue {revenue}, profit {profit}")
                quantity = 0  # 매도 후 수량 초기화
            # OCL 값이 음수에서 양수로 변하는 시점 (매수 시점)
            elif ocl < 0 and next_ocl > 0:
                buy_price = df.iloc[i]
                quantity = final_capital / buy_price
                trades.append({
                    "type": "buy",
                    "date": candle_data[i]['candle_date_time_kst'],
                    "price": buy_price,
                ...
                print(f"매수 at {candle_data[i]['candle_date_time_kst']} with price {buy_price}, quantity {quantity}")

            # 이전 OCL 값을 현재 OCL 값으로 업데이트
            
            # print(f"{candle_data[i]['candle_date_time_kst']} next_ocl: {next_ocl}, ocl: {ocl}, macd: {macd[i]}, exp3: {exp3[i]}")
            
            
            macd_list.append(
                {"type": "MACD", "DT": candle_data[i]['candle_date_time_kst'], "MACD": round(macd[i], 4), "SIGNAL": round(exp3[i], 4),
                 "OCL": ocl})
        
        # 최종 자본금 출력
        if quantity > 0:
            # 마지막에 보유 중인 주식 평가액을 더해 최종 자본 계산
            final_capital = quantity * df.iloc[0]  # 현재가로 평가
            print(f"최종 자본금 (보유 자산 평가액 포함): {final_capital}")
        else:
            print(f"최종 자본금: {final_capital}")
...
def get_macd2(target_item, tick_kind, inq_range):
    try:
